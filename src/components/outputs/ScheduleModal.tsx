...
import type { OutputSchedule, Report } from '../../types';

interface ScheduleModalProps {
  report: Report;
  existingSchedule?: OutputSchedule;
  onSave: (schedule: OutputSchedule) => void;
  onRemove: () => void;
  onClose: () => void;
}
...
export function ScheduleModal({ report, existingSchedule, onSave, onRemove, onClose }: ScheduleModalProps) {
  const [frequency, setFrequency] = useState<OutputSchedule['frequency']>(existingSchedule?.frequency ?? 'weekly');
  const [day, setDay] = useState(existingSchedule?.day ?? 'Mon');
  const [time, setTime] = useState(existingSchedule?.time ?? '09:00');

  function handleSave() {
    const schedule: OutputSchedule = { frequency, time };
    if (frequency === 'weekly') schedule.day = day;
    if (frequency === 'monthly') schedule.dayOfMonth = 1;
    onSave(schedule);
  }
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-[380px] rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] shadow-2xl overflow-hidden"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--color-border)]">
            <h3 className="text-sm font-semibold text-[var(--color-text-primary)] truncate pr-4">
                  ...
                  {days.map((d) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                        day === d
                          ? 'bg-[var(--color-accent)] text-white'
                          : 'bg-[var(--color-bg)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-elevated)] border border-[var(--color-border)]'
                      }`}
                    >
                      {d}
                    </button>
